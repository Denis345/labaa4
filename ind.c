...
//    }
    ...
    if ((node._index == 2) || (node._index == 6) || (node._index == 7) || (node._index == 8)){
        if (sigaction(SIGUSR1, &node._act, 0) == -1)
        failure("Can not change the action for child\n");
        
    }
    
     if ((node._index == 1) || (node._index == 2) || (node._index == 4) || (node._index == 5)){
        if (sigaction(SIGUSR2, &node._act, 0) == -1)
        failure("Can not change the action for child\n");
